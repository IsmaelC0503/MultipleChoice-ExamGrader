...
--------
    ...
    python main.py clave --hoja datos/maestra.jpg --salida datos/clave.json

    # 2. revisar que las fotos sirvan ANTES de calificar
    python main.py diagnostico --fotos datos/fotos

    # 3. calificar todo el lote
    python main.py calificar --fotos datos/fotos --clave datos/clave.json \
                             --padron datos/alumnos.xlsx --examen "Final 2026"
"""
...
def listar_fotos(carpeta):
    fotos = sorted(sum([glob.glob(os.path.join(carpeta, p)) for p in PATRONES], []))
    if not fotos:
