"""
Calificador automatico de examenes de opcion multiple — aplicacion de escritorio.

    python app.py

Universidad Nacional de Piura · Procesamiento Digital de Senales 2

Hace lo mismo que la interfaz del cuaderno pero como programa: seleccionar la
carpeta de fotos, la clave y el padron, calificar el lote, revisar hoja por
hoja, corregir a mano y exportar a CSV/Excel.
"""
...
def listar_fotos(carpeta):
    """
    Lista las imagenes de una carpeta SIN repetir.

    En Windows y macOS el sistema de archivos no distingue mayusculas, asi que
    los patrones '*.jpeg' y '*.JPEG' devuelven los mismos archivos y cada foto
    aparecia dos veces. Se normaliza la ruta para quedarse con una sola.
    """
    vistos, salida = set(), []
...
def cv2_a_tk(img, ancho_max=900, alto_max=700):
    """Convierte una imagen de OpenCV a algo que Tkinter pueda mostrar."""
    h, w = img.shape[:2]
...
class MarcoScroll(ttk.Frame):
    ...
    Hace falta porque en pantallas de 768 px de alto (muy comunes en portatiles)
    el contenido de la primera pestana no cabe y el boton de calificar quedaba
    fuera de la ventana, sin forma de bajar.
    """
...
class VisorImagen(tk.Toplevel):
    """Ventana aparte para ver una imagen en grande, con zoom y desplazamiento."""
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # la ventana se adapta a la pantalla: en portatiles de 1366x768 una
        # ventana de 800 px de alto se sale por abajo
        ...
        self.var_fotos = tk.StringVar(value=os.path.abspath('datos/fotos'))
        self.var_salida = tk.StringVar(value=os.path.abspath('resultados'))
        self.var_padron = tk.StringVar(value=os.path.abspath('datos/alumnos.xlsx'))
    ...
    def _barra_accion(self):
        """Barra fija abajo: el boton de calificar no puede quedar fuera de
        la ventana por mucho que se encoja la pantalla."""
    ...
    def _paso(self, padre, num, titulo, ayuda=None):
        """Recuadro numerado, para que se vea el orden en que hay que ir."""
    ...
    def procesar_lote(self):
        ...
        # IMPORTANTE: las variables de Tkinter solo se pueden leer desde el hilo
        # principal, asi que se resuelven aqui y se pasan como valores normales.
